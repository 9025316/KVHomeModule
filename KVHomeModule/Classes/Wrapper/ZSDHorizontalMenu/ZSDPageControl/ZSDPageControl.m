...
//圆点宏
#define kPageControlW 7
#define kPageControlH 6
#define kControlSpace 10

@interface ZSDPageControl ()


@end

@implementation ZSDPageControl
...
- (void)initialize {
    ...
    _numberOfPages = 0;
    _currentPage = 0;
    _controlSize    = kPageControlW;
    _controlHeight  = kPageControlH;
    _controlSpacing = kControlSpace;
...
- (void)createPointView {
    ...
    for (int page = 0; page < _numberOfPages; page++) {
        if (page == _currentPage) {
            UIView *currPointView = [[UIView alloc] initWithFrame:CGRectMake(startX, startY, _controlSize * 2, _controlHeight)];
...
- (void)exchangeCurrentView:(NSInteger)old new:(NSInteger)new {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        oldSelect.frame=CGRectMake(lx, mpSelect.origin.y, self->_controlSize, self->_controlHeight);
        
        CGFloat mx=newTemp.origin.x;
        if(new>old)
            mx-=self->_controlSize;
        newSeltect.frame=CGRectMake(mx, newTemp.origin.y, self->_controlSize*2, self->_controlHeight);
        ...
        if(new-old>1)
        {
            for(NSInteger t=old+1;t<new;t++)
            {
                ...
                ms.frame=CGRectMake(ms.frame.origin.x-self->_controlSize, ms.frame.origin.y, self->_controlSize, self->_controlHeight);
            }
        }
        // 右边选中到左边的时候 越过点击
        if(new-old<-1)
